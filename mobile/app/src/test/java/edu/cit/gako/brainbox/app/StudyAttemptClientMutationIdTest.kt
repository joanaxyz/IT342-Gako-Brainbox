    ...
    fun quizAttemptUsesCallerMutationId() = runBlocking {
        ...
        val repository = QuizRepository(
            object : QuizApiService {
                override suspend fun getQuizzesEnvelope(): ApiEnvelope<List<NetworkQuizSummary>> = unused()
                ...
                ): ApiEnvelope<NetworkQuizDetail> = unused()

                override suspend fun deleteQuizEnvelope(uuid: String): ApiEnvelope<Unit> = unused()

                override suspend fun getQuizEnvelope(uuid: String): ApiEnvelope<NetworkQuizDetail> = unused()
                ...
                override suspend fun recordQuizAttemptEnvelope(
                    ...
                    return ApiEnvelope(
                        success = true,
                        data = NetworkQuizDetail(uuid = uuid, title = "Quiz")
                    )
                }
            }
        )
    ...
    fun flashcardAttemptUsesCallerMutationId() = runBlocking {
        ...
        val repository = FlashcardRepository(
            object : FlashcardApiService {
                ...
                ): ApiEnvelope<NetworkFlashcardDeckDetail> = unused()
                ...
                override suspend fun recordFlashcardAttemptEnvelope(
                    ...
                    return ApiEnvelope(
                        success = true,
                        data = NetworkFlashcardDeckDetail(uuid = uuid, title = "Deck")
                    )
                }
            }
        )
