package edu.cit.gako.brainbox.platform.persistence
...
class BrainBoxPreferencesStore(context: Context) {
    ...
    suspend fun setPlaybackQueueSnapshot(
        ownerKey: String?,
        playlistUuid: String?,
        ...
        update { mutablePrefs ->
            if (ownerKey.isNullOrBlank() || playlistUuid.isNullOrBlank()) {
                mutablePrefs.remove(Key.playbackQueueOwnerKey)
                mutablePrefs.remove(Key.playbackQueuePlaylistUuid)
                mutablePrefs.remove(Key.playbackQueuePlaylistTitle)
                mutablePrefs.remove(Key.playbackQueueCurrentIndex)
                mutablePrefs[Key.playbackQueueShuffle] = shuffle
                return@update
            }

            mutablePrefs[Key.playbackQueueOwnerKey] = ownerKey
            mutablePrefs[Key.playbackQueuePlaylistUuid] = playlistUuid
            if (playlistTitle.isNullOrBlank()) {
                mutablePrefs.remove(Key.playbackQueuePlaylistTitle)
            } else {
                mutablePrefs[Key.playbackQueuePlaylistTitle] = playlistTitle
            }
            mutablePrefs[Key.playbackQueueCurrentIndex] = currentIndex.coerceAtLeast(0)
            mutablePrefs[Key.playbackQueueShuffle] = shuffle
        }
    }

    suspend fun clearPlaybackQueueSnapshot() {
        update { mutablePrefs ->
            mutablePrefs.remove(Key.playbackQueueOwnerKey)
            mutablePrefs.remove(Key.playbackQueuePlaylistUuid)
            mutablePrefs.remove(Key.playbackQueuePlaylistTitle)
            mutablePrefs.remove(Key.playbackQueueCurrentIndex)
            mutablePrefs.remove(Key.playbackQueueShuffle)
        }
    }
    ...
    private fun Preferences.toAppPlayerPreferences(): AppPlayerPreferences {
        return AppPlayerPreferences(
            ...
            playbackQueueOwnerKey = this[Key.playbackQueueOwnerKey],
            playbackQueuePlaylistUuid = this[Key.playbackQueuePlaylistUuid],
            playbackQueuePlaylistTitle = this[Key.playbackQueuePlaylistTitle],
            playbackQueueCurrentIndex = this[Key.playbackQueueCurrentIndex] ?: 0,
            playbackQueueShuffle = this[Key.playbackQueueShuffle] ?: false,
    ...
    private object Key {
        ...
        val playbackQueueOwnerKey = stringPreferencesKey("playback_queue_owner_key")
        val playbackQueuePlaylistUuid = stringPreferencesKey("playback_queue_playlist_uuid")
        val playbackQueuePlaylistTitle = stringPreferencesKey("playback_queue_playlist_title")
        val playbackQueueCurrentIndex = intPreferencesKey("playback_queue_current_index")
        val playbackQueueShuffle = booleanPreferencesKey("playback_queue_shuffle")
