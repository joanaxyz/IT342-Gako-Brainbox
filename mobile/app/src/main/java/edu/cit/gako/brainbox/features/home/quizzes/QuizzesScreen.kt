        ...
        if (filtered.isEmpty()) {
            EmptyStateCard(
                ...
                body = if (query.isNotBlank() || hasActiveFilter) {
                    ...
                    "Quiz sets created on the web will appear here once the API sync completes."
                }
            )
        } else {
                                                ...
                                                if (isOffline) {
                                                    ...
                                                    offlinePackRepository.downloadNotebookPack(targetNotebookUuid).onSuccess { result ->
                                                        offlineMessage = if (result.audioPrepared) {
                                                            "\"${quiz.title}\" is now available offline with its notebook pack and audio ready."
                                                        } else {
                                                            "\"${quiz.title}\" is now available offline with its notebook pack, but audio could not be prepared yet."
                                                        }
                                                    }.onFailure {
