...
mobile flows, UI state, offline behavior, and Android infrastructure.
...
- `edu.cit.gako.brainbox.app.playback`
  - App-level playback orchestration that maps notebook, playlist, auth,
    local storage, and playback state into feature-owned playback contracts.
- `edu.cit.gako.brainbox.app.study`
  - Cross-feature study-session orchestration, such as opening quiz/deck
    sessions, queuing offline attempts, and returning to notebooks.
...
- `edu.cit.gako.brainbox.platform`
  - Generic Android infrastructure only: Retrofit/session setup, network
    envelope support, Room database setup, connectivity, preferences, and
    reusable Android services.
...
- `features.notebook`
  - Notebook data contracts, notebook DTOs, embedded web editor host, and
    notebook offline bundles. The editor/review UI is the web surface shipped
    as embedded assets; old native editor/review/AI panes should not return.
- `features.playback`
  - Audio service/client/store live under `audio`, playback models under
    `model`, TTS and offline audio cache under `tts`, player/queue Compose UI
    under `ui`, and REST queue data under `data`.
...
- Feature-specific Room entities, mappers, offline bundle handling, and sync
  replay logic belong to the owning feature or to app-level orchestration. The
  platform database can provide generic storage plumbing, but it does not own
  product behavior.
