...
- Name: 07-notebook-ai.spec.mjs >> NOTEBOOK — AI Features (Detailed) >> WEB-NB-AI-011: AI Proposal Overlay (if triggered)
- Location: tests\e2e\07-notebook-ai.spec.mjs:190:3
    ...
    - generic [ref=e87]:
      ...
      - generic [ref=e224]:
        ...
        - generic [ref=e239]:
          ...
          - button "Playwright Test Notebook Ready to be sorted into a category Uncategorized 0 words 5m ago Open" [ref=e245] [cursor=pointer]:
            - generic:
              - img [ref=e247]
              - generic:
            ...
            - generic [ref=e253]: 5m ago
            - button "Open" [active] [ref=e255]:
              - text: Open
              - img [ref=e256]
          - button "Playwright Test Notebook Ready to be sorted into a category Uncategorized 0 words 8m ago Open" [ref=e258] [cursor=pointer]:
            - generic:
              - img [ref=e260]
              - generic:
                - generic: Playwright Test Notebook
...
```ts
  ...
  170 |   });
  171 | 
  172 |   test('WEB-NB-AI-010: AI Sidebar — Quiz tool', async ({ page }) => {
  173 |     await page.goto('/library');
  174 |     await page.waitForTimeout(3000);
  175 |     await page.locator('.lib-row').first().locator('text=Open').click();
  176 |     await page.waitForTimeout(4000);
  177 |     
  178 |     const aiBtn = page.locator('button[title*="AI"]').first();
  179 |     await aiBtn.click();
  180 |     await page.waitForTimeout(2000);
  181 |     
  182 |     const quizTool = page.locator('.ai-sidebar-tool, [data-tool]').filter({ hasText: /quiz/i }).first();
  183 |     if (await quizTool.isVisible({ timeout: 2_000 }).catch(() => false)) {
  184 |       await quizTool.click();
  185 |       await page.waitForTimeout(1000);
  186 |     }
  187 |     await snap(page, 'WEB-NB-AI-010_quiz-tool');
  188 |   });
  189 | 
  190 |   test('WEB-NB-AI-011: AI Proposal Overlay (if triggered)', async ({ page }) => {
  191 |     await page.goto('/library');
  192 |     await page.waitForTimeout(3000);
  193 |     await page.locator('.lib-row').first().locator('text=Open').click();
  194 |     await page.waitForTimeout(4000);
  195 |     
  196 |     // Open AI sidebar
  197 |     const aiBtn = page.locator('button[title*="AI"]').first();
> 198 |     await aiBtn.click();
      |                 ^ TimeoutError: locator.click: Timeout 15000ms exceeded.
  199 |     await page.waitForTimeout(2000);
  200 |     
  201 |     await snap(page, 'WEB-NB-AI-011_ai-sidebar-overview');
  202 |   });
  203 | 
  204 |   test('WEB-NB-AI-012: AI Sidebar closes', async ({ page }) => {
  205 |     await page.goto('/library');
  206 |     await page.waitForTimeout(3000);
  207 |     await page.locator('.lib-row').first().locator('text=Open').click();
  208 |     await page.waitForTimeout(4000);
  209 |     
  210 |     const aiBtn = page.locator('button[title*="AI"]').first();
  211 |     await aiBtn.click();
  212 |     await page.waitForTimeout(2000);
  213 |     await snap(page, 'WEB-NB-AI-012a_ai-sidebar-open');
  214 |     
  215 |     // Click close button
  216 |     const closeBtn = page.locator('.ai-sidebar-close, button[aria-label*="close"]').first();
  217 |     if (await closeBtn.isVisible({ timeout: 2_000 }).catch(() => false)) {
  218 |       await closeBtn.click();
  219 |       await page.waitForTimeout(1000);
  220 |       await snap(page, 'WEB-NB-AI-012b_ai-sidebar-closed');
  221 |     }
  222 |   });
  223 | });
  224 | 
